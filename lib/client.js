window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var REALTIME_WORKLET_CODE = [
      ...
      '    if (input && input[0]) {',
      '      const data = input[0];',
      '      for (let i = 0; i < data.length; i++) this.src.push(data[i]);',
      '      this.emitResampled();',
      '    }',
      '    if (this.stopped) {',
      '      if (this.buffer.length > 0) { this.port.postMessage(new Int16Array(this.buffer)); this.buffer = []; }',
    ...
    function VoiceInputButton(props) {
      ...
      function finishRealtime(rt) {
        ...
        try { if (rt.audioContext !== null) rt.audioContext.close() } catch (_err) { /* ignore */ }
        if (!aliveRef.current) return
        var raw = String(rt.finalText || '').trim() === '' ? String(rt.lastText || '') : String(rt.finalText || '')
              ...
              if (event.data instanceof Int16Array) {
                if (rt.socket !== null && rt.socket.readyState === 1) {
                  try {
                    if (rt.protocol === 'fun-asr') {
                      if (rt.taskStarted) {
                        rt.socket.send(event.data.buffer)
                        rt.audioChunks += 1
                      }
                    } else {
                      rt.socket.send(JSON.stringify({
            ...
            socket.onmessage = function (event) {
              ...
              if (rt.protocol === 'fun-asr') {
                ...
                if (header.event === 'task-started') {
                  ...
                  if (sentence !== null && sentence !== undefined && typeof sentence.text === 'string') {
                    rt.lastText = sentence.text.trim()
                    if (rt.lastText !== '' && (sentence.end_time !== null && sentence.end_time !== undefined || sentence.sentence_end === true)) {
                      rt.finalText += (rt.finalText === '' ? '' : ' ') + rt.lastText
                    }
                  }
