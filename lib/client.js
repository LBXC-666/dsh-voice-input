window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      /* 录音蓝色波纹 / 整理彩虹流光：沿边框旋转；蓝色增加上下起伏 + 音量震动 */
      '.vi_flow-host{--vi-wave-scale:1;--vi-wave-shake:0px;position:relative}',
      '.vi_flow-overlay{position:absolute;inset:-4px;z-index:30;pointer-events:none;border-radius:inherit;padding:4px;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;overflow:hidden}',
      '.vi_flow-overlay[data-flow=blue]{animation:vi_ripple_shake .6s ease-in-out infinite}',
      '@property --vi-angle{syntax:"<angle>";initial-value:0deg;inherits:false}',
      '.vi_flow-wave{position:absolute;inset:0}',
      '.vi_flow-wave-a{background:repeating-conic-gradient(from var(--vi-angle,0deg),rgba(10,132,255,.12) 0deg 22deg,rgba(10,132,255,.95) 22deg 28deg,rgba(90,209,255,.8) 28deg 32deg,rgba(10,132,255,.12) 32deg 60deg);animation:vi_flow_spin 2.4s linear infinite}',
      '.vi_flow-wave-b{background:repeating-linear-gradient(108deg,transparent 0 26px,rgba(0,176,255,.55) 26px 30px,rgba(0,120,220,.35) 30px 34px,transparent 34px 70px);background-size:260% 100%;animation:vi_flow_b 2.6s linear infinite}',
      '.vi_flow-overlay[data-flow=blue] .vi_flow-wave-a{animation:vi_flow_spin 2.4s linear infinite,vi_wave_bob_a 1.1s ease-in-out infinite}',
      '.vi_flow-overlay[data-flow=blue] .vi_flow-wave-b{animation:vi_flow_b 2.6s linear infinite,vi_wave_bob_b 1.7s ease-in-out infinite}',
      '@keyframes vi_wave_bob_a{0%,100%{transform:translateY(-2.5px)}50%{transform:translateY(2.5px)}}',
      '@keyframes vi_wave_bob_b{0%,100%{transform:translateY(2px)}50%{transform:translateY(-3px)}}',
      ...
      '@keyframes vi_ripple_shake{0%,100%{transform:translateX(calc(var(--vi-wave-shake,0px)*-1)) translateY(-3.5px) scale(var(--vi-wave-scale,1))}25%{transform:translateX(var(--vi-wave-shake,0px)) translateY(0) scale(var(--vi-wave-scale,1))}50%{transform:translateX(calc(var(--vi-wave-shake,0px)*-1)) translateY(3.5px) scale(var(--vi-wave-scale,1))}75%{transform:translateX(var(--vi-wave-shake,0px)) translateY(0) scale(var(--vi-wave-scale,1))}}',
            ...
            node.port.onmessage = function (event) {
              ...
              if (event.data !== null && typeof event.data === 'object' && event.data.type === 'level') {
                ...
                if (host !== null) {
                  ...
                  host.style.setProperty('--vi-wave-scale', (1 + normalized * 0.08).toFixed(4))
                  host.style.setProperty('--vi-wave-shake', (normalized * 8).toFixed(2) + 'px')
                }
                return
              }
