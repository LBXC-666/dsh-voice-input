window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      /* 折叠式分类面板 */
      '.vi_section{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;overflow:hidden}',
      '.vi_sectionSummary{list-style:none;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 16px;cursor:pointer;user-select:none}',
      '.vi_sectionSummary::-webkit-details-marker{display:none}',
      '.vi_sectionSummaryHead{min-width:0;display:flex;flex-direction:column;gap:2px}',
      '.vi_sectionSummaryTitle{font-size:13px;font-weight:600;line-height:20px;color:var(--dsw-alias-label-primary)}',
      '.vi_sectionSummaryDesc{font-size:12px;line-height:18px;color:var(--dsw-alias-label-tertiary)}',
      '.vi_sectionSummary::after{content:"";flex:none;width:8px;height:8px;border-right:1.6px solid var(--dsw-alias-label-tertiary);border-bottom:1.6px solid var(--dsw-alias-label-tertiary);transform:rotate(45deg);transition:transform .18s var(--ds-ease-in-out)}',
      '.vi_section[open]>.vi_sectionSummary::after{transform:rotate(-135deg)}',
      '.vi_section[open]>.vi_sectionSummary{border-bottom:1px solid var(--dsw-alias-border-l2)}',
      '.vi_sectionBody{display:flex;flex-direction:column;gap:12px;padding:14px 16px 16px}',
      '.vi_subTitle{margin:0;font-size:12px;font-weight:600;line-height:18px;color:var(--dsw-alias-label-secondary);padding-top:2px}',
      ...
      /* AI 整理期间：文本流光扫描。优先按“实际有文字的区域”逐行扫过；
         无法测量文字时回退为整行扫过（备选方案） */
      '.vi_scan-host{position:relative}',
      '.vi_text-scan{position:absolute;inset:0;z-index:4;pointer-events:none;overflow:hidden;border-radius:inherit}',
      '.vi_text-scan::before{content:"";position:absolute;top:-6px;bottom:-6px;left:0;width:26%;background:linear-gradient(90deg,transparent 0%,rgba(0,150,255,.04) 18%,rgba(110,195,255,.14) 38%,rgba(255,255,255,.42) 50%,rgba(110,195,255,.14) 62%,rgba(0,150,255,.04) 82%,transparent 100%);filter:blur(3px);transform:translateX(-130%);animation:vi_text_scan 1.9s var(--ds-ease-in-out) infinite}',
      '.vi_text-scan::after{content:"";position:absolute;top:0;bottom:0;left:0;width:55%;background:linear-gradient(90deg,transparent 0%,rgba(0,120,255,.06) 20%,rgba(120,200,255,.12) 50%,rgba(0,120,255,.06) 80%,transparent 100%);filter:blur(6px);transform:translateX(-120%);animation:vi_text_scan_soft 1.9s var(--ds-ease-in-out) .5s infinite}',
      '@keyframes vi_text_scan{0%{transform:translateX(-130%);opacity:.15}18%{opacity:.95}82%{opacity:.95}100%{transform:translateX(430%);opacity:0}}',
      '@keyframes vi_text_scan_soft{0%{transform:translateX(-120%);opacity:0}25%{opacity:.8}85%{opacity:.8}100%{transform:translateX(230%);opacity:0}}',
      /* 已测量出文字范围时：关闭整行回退光带，改用逐行文本区域光带 */
      '.vi_scan-lines::before,.vi_scan-lines::after{display:none}',
      '.vi_scan-line{position:absolute;overflow:hidden;pointer-events:none;border-radius:4px}',
      '.vi_scan-line::before{content:"";position:absolute;inset:0;width:55%;background:linear-gradient(90deg,transparent 0%,rgba(0,150,255,.04) 22%,rgba(110,195,255,.14) 42%,rgba(255,255,255,.5) 52%,rgba(110,195,255,.14) 62%,rgba(0,150,255,.04) 78%,transparent 100%);filter:blur(2px);transform:translateX(-110%);animation:vi_scan_line 1.9s var(--ds-ease-in-out) infinite}',
      '.vi_scan-line::after{content:"";position:absolute;inset:0;width:85%;background:linear-gradient(90deg,transparent 0%,rgba(0,120,255,.05) 20%,rgba(120,200,255,.12) 50%,rgba(0,120,255,.05) 80%,transparent 100%);filter:blur(4px);transform:translateX(-130%);animation:vi_scan_line_soft 1.9s var(--ds-ease-in-out) .45s infinite}',
      '@keyframes vi_scan_line{0%{transform:translateX(-110%);opacity:.1}20%{opacity:.95}80%{opacity:.95}100%{transform:translateX(230%);opacity:0}}',
      '@keyframes vi_scan_line_soft{0%{transform:translateX(-130%);opacity:0}30%{opacity:.75}85%{opacity:.75}100%{transform:translateX(170%);opacity:0}}',
      '.vi_scan-mirror{position:absolute;opacity:0;pointer-events:none;white-space:pre-wrap;word-break:break-word;overflow-wrap:break-word;box-sizing:border-box;z-index:-1;left:0;top:0}',
      '.vi_scan-mirror span{white-space:inherit;word-break:inherit;overflow-wrap:inherit}',
      /* 转写文本插入输入框时的闪现动画 */
    ...
    var zh = {
      tab: '语音输入',
      settingsTitle: '语音输入',
      settingsIntro: '在输入框发送键左侧启用语音输入。各分类可点击标题单独展开或折叠，配置会自动保存在当前浏览器中。',
      basicsGroup: '基础设置',
      basicsSummary: '选择识别方式与语音输入开关',
      providerGroup: '识别接口参数',
      providerSummary: '接口地址、密钥、模型与高级参数（随上方接口模式变化）',
      formatterTriggerSub: '触发方式',
      formatterModelSub: '模型接口与思考',
      formatterPromptsSub: '提示词（只有一轮与二轮）',
      providerFieldsSub: '接口参数',
      mode: '接口模式',
      ...
      micChannelPrefix: '声道',
      formatterGroup: '文本整理（AI 优化）',
      formatterSummary: '自动优化、按钮行为、模型思考档位与提示词',
      formatterIntro: '语音识别完成后可自动优化（由上面的开关控制），也可以点 ✨ 手动优化；左键 / 右键各自执行哪一轮由下方模式决定。结果自动写回输入框。',
      optimizeAria: 'AI 优化文本',
      optimizeTitle: 'AI 优化文本（左键、右键执行模式见设置）',
      ...
      formatterFixTemplate: '一轮提示词（纠错）',
      formatterFixTemplateHint: '第一轮：只纠正错别字/口语化/识别错误，输出干净文本；可用 {{context}} 与 {{text}} 占位符。',
      formatterFinalTemplate: '二轮提示词（固定格式）',
      formatterFinalTemplateHint: '第二轮：把纠错后的文本整理成固定格式，不注入上下文；可用 {{text}} 占位符。',
      formatterThinking: '模型思考档位',
      formatterThinkingHint: '默认低：低档思考可提高输出准确性；关闭则最快。兼容 DeepSeek thinking 与 DashScope enable_thinking。',
      thinkingOff: '关闭思考',
      thinkingLow: '低（推荐）',
      thinkingMedium: '中',
      thinkingHigh: '高',
      autoOptimizeEnabled: '自动优化开关',
      autoOptimizeEnabledHint: '开启后，语音识别完成即自动执行「自动优化模式」，并用优化结果替换刚转写的原文，不需要任何快捷键。',
      clickLeftMode: '左键模式',
      clickLeftModeHint: '左键 ✨ 执行什么；「一轮 + 二轮」表示先纠错再格式化。',
      clickRightMode: '右键模式',
      clickRightModeHint: '右键 ✨ 执行什么；「一轮 + 二轮」表示先纠错再格式化。',
      autoMode: '自动优化模式',
      autoModeHint: '自动优化开关打开后，语音转写完成自动执行的流程。',
      modeRound1: '一轮（纠错）',
      modeRound2: '二轮（固定格式）',
      modeRound1Plus2: '一轮 + 二轮',
      rollbackAria: '回退到优化前文本',
      rollbackTitle: '回退到优化前文本',
      formatterWithContext: '注入当前对话上下文',
      formatterWithContextHint: '把最近 2 条对话文本完整附加（超长时只保留结尾 800 字，避免丢掉最新结论），帮助理解指代与联想；不发送附件。',
      formatterNote: '每一轮整理在后台调用一次 chat/completions；结果直接替换输入框内容，可点 ↶ 回退。',
      ...
      shortcutEnabled: '启用右 Alt 语音输入',
      shortcutHint: '语音输入唯一的全局快捷键：按住右 Alt 说话，松手即停止并转写。AltGr 键盘布局、组合键与长按重复会被自动忽略。',
      configure: '去配置',
      errOpenSettings: '无法自动打开设置面板，请点击左下角设置按钮，进入「语音输入」。',
      importGroup: '导入 / 导出 API 配置',
      importSummary: '粘贴或选择 JSON 文件导入；复制或下载当前配置',
    ...
    var en = {
      ...
      settingsIntro: 'Adds a voice input button to the left of the send button. Each category can be expanded or collapsed by clicking its title; the configuration is saved in this browser.',
      basicsGroup: 'Basics',
      basicsSummary: 'Pick a recognition mode and the voice input shortcut',
      providerGroup: 'Recognition endpoint',
      providerSummary: 'Endpoint, key, model and advanced parameters for the selected mode',
      formatterTriggerSub: 'When it runs',
      formatterModelSub: 'Model, key and thinking',
      formatterPromptsSub: 'Prompts (only round 1 and round 2)',
      providerFieldsSub: 'Endpoint parameters',
      mode: 'Provider mode',
      ...
      formatterGroup: 'Text formatting (AI optimize)',
      formatterSummary: 'Auto optimize, button behavior, thinking level and prompts',
      formatterIntro: 'After speech recognition the text can be optimized automatically (controlled by the auto-optimize switch above) or manually with ✨; the left/right-click modes below decide which round each runs. The result is written back automatically.',
      optimizeAria: 'Optimize text with AI',
      optimizeTitle: 'Optimize text with AI (left/right click behavior is set in settings)',
      ...
      formatterFixTemplate: 'Round-1 prompt (fix)',
      formatterFixTemplateHint: 'Round 1: fix typos, punctuation and obvious colloquial/ASR errors only, and output clean text. Supports {{context}} and {{text}} placeholders.',
      formatterFinalTemplate: 'Round-2 prompt (fixed format)',
      formatterFinalTemplateHint: 'Round 2: turns the corrected text into the fixed format without context. Supports the {{text}} placeholder.',
      formatterThinking: 'Model thinking level',
      formatterThinkingHint: 'Defaults to low: a little thinking improves accuracy; off is fastest. Works with DeepSeek thinking and DashScope enable_thinking.',
      thinkingOff: 'Off',
      thinkingLow: 'Low (recommended)',
      thinkingMedium: 'Medium',
      thinkingHigh: 'High',
      autoOptimizeEnabled: 'Auto optimize',
      autoOptimizeEnabledHint: 'When enabled, each completed transcription is optimized automatically with the auto mode and the raw transcript is replaced by the optimized text; no hotkeys are needed.',
      clickLeftMode: 'Left-click mode',
      clickLeftModeHint: 'What the left-click ✨ does; "round 1 + round 2" fixes first and then formats.',
      clickRightMode: 'Right-click mode',
      clickRightModeHint: 'What the right-click ✨ does; "round 1 + round 2" fixes first and then formats.',
      autoMode: 'Auto optimize mode',
      autoModeHint: 'The flow that runs automatically after each transcription when auto optimize is enabled.',
      modeRound1: 'Round 1 (fix)',
      modeRound2: 'Round 2 (format)',
      modeRound1Plus2: 'Round 1 + round 2',
      rollbackAria: 'Rollback to text before optimizing',
      rollbackTitle: 'Rollback to text before optimizing',
      formatterWithContext: 'Include current conversation context',
      formatterWithContextHint: 'Appends the latest 2 user/assistant messages in full (overlong messages keep their last 800 chars so recent conclusions are not lost); attachments are not sent.',
      formatterNote: 'Each formatting round calls chat/completions once in the background and replaces the composer content; use ↶ to roll back.',
      ...
      shortcutEnabled: 'Enable Right Alt voice input',
      shortcutHint: 'The only global shortcut: hold Right Alt to talk, release to stop and transcribe. AltGr layouts, key combos and key-repeat are ignored automatically.',
      configure: 'Configure',
      ...
      importSummary: 'Paste or pick a JSON file to import; copy or download the current configuration',
    ...
    var DEFAULT_FORMATTER_FIX_TEMPLATE = [
      ...
      '上下文输入截止',
      '',
      '请只纠正下面文本中的错别字、标点和明显口语化错误，或语音识别错误，保持原意和结构，去除‘额’、‘嗯’等语气词，不要改写、不要解释，直接输出纠正后的文本：',
      '{{text}}',
    ].join('\n')

    var DEFAULT_FORMATTER_FINAL_TEMPLATE = [
      '请对下面的原始输入做格式化。',
      '任务：提取用户的真正目标、所有关键要求和约束，但不要遗漏任何细节。',
      '但不要编造上下文里没有的信息。',
      '按下面的格式输出（只输出整理后的内容，不要任何解释；仅匹配到相关内容时输出对应格式（如：我想完成xxx任务就是目标，修复xxx就是问题））：',
      '',
      ' 示例：',
      '      先一句话说明用户想达成什么',
      '      目标：',
    ...
    var DEFAULT_CONFIG = {
      ...
      formatterFinalTemplate: DEFAULT_FORMATTER_FINAL_TEMPLATE,
      formatterThinking: 'low', // off | low | medium | high
    ...
    function normalizeConfig(raw) {
      ...
      // 新版只保留一轮（纠错）与二轮（固定格式）两个提示词；
      // 旧版单一 formatterTemplate 迁移为二轮格式化提示词。
      base.formatterFixTemplate = asString(src.formatterFixTemplate, base.formatterFixTemplate)
      base.formatterFinalTemplate = asString(
        src.formatterFinalTemplate,
        asString(src.formatterTemplate, base.formatterFinalTemplate),
      )
      var thinkingValues = ['off', 'low', 'medium', 'high']
      if (typeof src.formatterThinking === 'string') {
        base.formatterThinking = thinkingValues.indexOf(src.formatterThinking) >= 0 ? src.formatterThinking : base.formatterThinking
      } else if (typeof src.formatterNoThinking === 'boolean') {
        // 旧配置迁移：老版「关闭模型思考」布尔开关 → 新档位（关→off，开→low）
        base.formatterThinking = src.formatterNoThinking ? 'off' : 'low'
      }
      base.autoOptimizeEnabled = typeof src.autoOptimizeEnabled === 'boolean' ? src.autoOptimizeEnabled : base.autoOptimizeEnabled
      var modeValues = ['round1', 'round2', 'round1+round2']
      base.clickLeftMode = modeValues.indexOf(src.clickLeftMode) >= 0 ? src.clickLeftMode : base.clickLeftMode
      base.clickRightMode = modeValues.indexOf(src.clickRightMode) >= 0 ? src.clickRightMode : base.clickRightMode
      base.autoMode = modeValues.indexOf(src.autoMode) >= 0 ? src.autoMode : base.autoMode
      // 旧版「启用第二轮优化」开关已移除：选择「一轮」本身就等于旧版关闭二轮；
      // 老配置若关闭过二轮，则把「一轮 + 二轮」降级为「一轮」，保持原有行为
      if (src.formatterTwoPassEnabled === false) {
        if (base.clickLeftMode === 'round1+round2') base.clickLeftMode = 'round1'
        if (base.clickRightMode === 'round1+round2') base.clickRightMode = 'round1'
        if (base.autoMode === 'round1+round2') base.autoMode = 'round1'
      }
      if (typeof src.formatterNoThinking === 'boolean') base.formatterNoThinking = src.formatterNoThinking
    ...
    function getComposerCaret(fallback) {
      if (typeof document === 'undefined') return fallback || 0
      var textarea = document.querySelector('textarea[data-phase]')
      if (textarea === null) return fallback || 0
      try {
        return typeof textarea.selectionStart === 'number' ? textarea.selectionStart : (fallback || 0)
      } catch (_err) {
        return fallback || 0
      }
    }
    ...
    function focusComposer(caret) {
      ...
      window.setTimeout(function () {
        ...
        try {
          element.focus()
          var end = typeof caret === 'number' ? caret : element.value.length
    ...
    function conversationContext(snapshot) {
      ...
      var lines = []
      var count = 0
      for (var i = snapshot.nodes.length - 1; i >= 0 && count < 2; i--) {
    ...
    function buildFormatPayload(model, content, thinkingLevel) {
      var payload = {
        model: model,
        temperature: 0.2,
        stream: false,
      ...
      // 兼容：DashScope/Qwen 用 enable_thinking，DeepSeek 用 thinking 对象，
      // 通用 OpenAI 系用 reasoning_effort；关闭时不发送思考参数外的多余字段。
      if (thinkingLevel === 'off') {
        payload.enable_thinking = false
        payload.thinking = { type: 'disabled' }
      } else if (thinkingLevel === 'plain') {
        // 最简降级请求：完全不带思考相关字段，兼容严格的 OpenAI 兼容服务
      } else {
    ...
    function formatDraftText(text, contextText, cfg, t) {
      ...
      var thinkingLevel = ['off', 'low', 'medium', 'high'].indexOf(cfg.formatterThinking) >= 0
        ? cfg.formatterThinking
        : 'low'
      var url = endpoint.replace(/\/+$/, '') + '/chat/completions'

      var attempt = function (content, level) {
        return requestChatCompletions(url, key, buildFormatPayload(model, content, level), t)
      }
      ...
      // 依次降级：上下文/思考参数可能被某些模型拒绝（400/422）
      // 1. 完整上下文 + 所选思考档位 → 2. 无上下文 + 所选档位 → 3. 完整上下文 + 关闭思考
      // → 4. 无上下文 + 关闭思考 → 5. 最简请求（不带任何思考字段）
      var fallback = []
      fallback.push({ content: contentFull, level: thinkingLevel })
      if (useContext) fallback.push({ content: contentMinimal, level: thinkingLevel })
      if (thinkingLevel !== 'off') {
        fallback.push({ content: contentFull, level: 'off' })
        if (useContext) fallback.push({ content: contentMinimal, level: 'off' })
      }
      fallback.push({ content: useContext ? contentMinimal : contentFull, level: 'plain' })

      var index = 0
      var nextAttempt = function () {
        var item = fallback[index]
        index += 1
        return attempt(item.content, item.level).catch(function (error) {
          if (error !== null && (error.status === 400 || error.status === 422) && index < fallback.length) return nextAttempt()
          throw error
        })
      }
      return nextAttempt()
    ...
    function VoiceInputButton(props) {
      ...
      var rollbackRef = react.useRef(null)
      ...
      controllerRef.current = {
        start: startFromShortcut,
        stop: stopFromShortcut,
        format: formatDraft,
        optimize: runOptimizeMode,
      }
      react.useEffect(function () {
      ...
      // 文本发送/清空后，回退快照立即失效：既隐藏回退按钮，
      // 也避免用户之后输入新文本时误回退到发送前的内容
      react.useEffect(function () {
        if (composerDraft.trim() === '' && rollbackRef.current !== null) {
      ...
      // AI 优化期间（等待模型返回 + 打字机写回）：优先按“实际有文字的区域”
      // 逐行扫过流光；无法测量文字范围时回退为整行光带（备选方案）
      var aiScanning = formatBusy || typing
      react.useEffect(function () {
        if (!aiScanning) return undefined
        var textarea = document.querySelector('textarea[data-phase]')
        if (textarea === null) return undefined
        var host = textarea.parentElement
        if (host === null) return undefined
        host.classList.add('vi_scan-host')

        var overlay = document.createElement('div')
        overlay.className = 'vi_text-scan'
        overlay.setAttribute('aria-hidden', 'true')
        host.appendChild(overlay)

        var mirror = document.createElement('div')
        mirror.className = 'vi_scan-mirror'
        ...
        function clearLineBoxes() {
          ...
          overlay.classList.remove('vi_scan-lines')
        }

        function copyMirrorStyle() {
          ...
          var keys = [
            ...
            'tab-size', 'line-height', 'word-break', 'overflow-wrap', 'white-space',
            'text-align', 'direction',
          ]
          for (var i = 0; i < keys.length; i++) {
            try { mirror.style[keys[i]] = computed[keys[i]] } catch (_err) { /* ignore */ }
          }
          mirror.style.left = (taRect.left - hostRect.left + parseFloat(computed.borderLeftWidth)) + 'px'
          mirror.style.top = (taRect.top - hostRect.top + parseFloat(computed.borderTopWidth)) + 'px'
          // clientWidth/clientHeight 不包含边框和滚动条，镜像内容宽度与文本换行严格一致
        ...
        function renderLineBoxes() {
          var value = textarea.value || ''
          var scrollTop = textarea.scrollTop || 0
          var width = textarea.clientWidth
          var height = textarea.clientHeight
          // 文本、滚动和尺寸都没变时不要重建节点，避免动画被 120ms 轮询打断
          if (value === lastValue && scrollTop === lastScrollTop && width === lastWidth && height === lastHeight) return
          lastValue = value
          ...
          try {
            copyMirrorStyle()
            // 超长文本退化为整行扫过，避免逐字测量拖慢页面
            if (value.length === 0 || value.length > 2000) {
              clearLineBoxes()
              return
            }
            var tokens = value.match(/\S+|\s+/g)
            if (tokens === null) {
              clearLineBoxes()
              return
            }
            ...
            for (var s = 0; s < spans.length; s++) {
              var rects = spans[s].getClientRects()
              for (var r = 0; r < rects.length; r++) {
                var rect = rects[r]
                if (rect.width > 0.5 && rect.height > 0.5) {
                  fragments.push({ top: rect.top, left: rect.left, right: rect.right, bottom: rect.bottom })
                }
              }
            }
            clearLineBoxes()
            ...
            for (var f = 0; f < fragments.length; f++) {
              var item = fragments[f]
              var group = groups.length > 0 ? groups[groups.length - 1] : null
              if (group === null || Math.abs(item.top - group.top) >= 1) {
                group = {
                  top: item.top,
                  bottom: item.bottom,
                  left: item.left,
                ...
                if (item.left < group.left) group.left = item.left
                if (item.right > group.right) group.right = item.right
                if (item.bottom > group.bottom) group.bottom = item.bottom
              }
            }
            for (var g = 0; g < groups.length; g++) {
              var box = groups[g]
              var left = box.left - hostRect.left
              var top = box.top - hostRect.top - scrollTop
              var width = box.right - box.left
              var height = box.bottom - box.top
              if (width < 2 || height < 2) continue
              var lineBox = document.createElement('div')
              lineBox.className = 'vi_scan-line'
              lineBox.style.left = left + 'px'
              lineBox.style.top = top + 'px'
              lineBox.style.width = width + 'px'
              lineBox.style.height = height + 'px'
              overlay.appendChild(lineBox)
            }
            if (overlay.firstChild !== null) overlay.classList.add('vi_scan-lines')
          } catch (_err) {
            clearLineBoxes()
          }
        }
        ...
        renderLineBoxes()
        timer = window.setInterval(renderLineBoxes, 120)

        return function () {
          if (timer !== null) window.clearInterval(timer)
          if (overlay.parentNode === host) host.removeChild(overlay)
          if (mirror.parentNode === host) host.removeChild(mirror)
          host.classList.remove('vi_scan-host')
        }
      }, [aiScanning])
      ...
      function insertCleanText(clean) {
        ...
        var current = liveInputRef.current && typeof liveInputRef.current.draft === 'string'
          ...
          : ''
        // 基于光标位置插入，而不是固定追加到末尾
        var caret = getComposerCaret(current.length)
        var prefix = current.slice(0, caret)
        var suffix = current.slice(caret)
        var sepBefore = prefix !== '' && !/[\s\n\u3000]$/.test(prefix) ? ' ' : ''
        var sepAfter = suffix !== '' && !/^[\s\n\u3000]/.test(suffix) ? ' ' : ''
        var next = prefix + sepBefore + clean + sepAfter + suffix
        actions.setDraft(next)
        var start = prefix.length + sepBefore.length
        focusComposer(start + clean.length)
        if (typeof document !== 'undefined') {
          var textarea = document.querySelector('textarea[data-phase]')
          if (textarea !== null) {
        ...
        // 返回刚插入文本的位置区间，供自动优化精准“原位替换”，不保留转写原文
        return { draft: next, start: start, end: start + clean.length }
      }
      ...
      function insertTranscript(text) {
        ...
        var inserted = insertCleanText(clean)
        // 自动优化开关：转写完成即按「自动优化模式」直接优化；
        // 用优化结果原位替换刚转写出来的原文，不保留原文
        if (cfg.autoOptimizeEnabled === true && inserted !== null) {
          runOptimizeMode(
            cfg.autoMode || 'round1+round2',
            inserted.draft,
            true,
            { start: inserted.start, end: inserted.end },
            clean,
          )
        }
      }

      function typewriterDraft(fullText, baseDraft, caret, replaceRange) {
        var actions = actionsRef.current
        if (actions === null || typeof actions.setDraft !== 'function') {
          setFormatBusy(false)
          setTyping(false)
        ...
        // 有替换区间（自动优化）：只替换刚转写的那一段，保留输入框里已有内容；
        // 没有区间（手动优化）：优化结果替换整个输入框，不保留原文。
        var range = replaceRange
        var hasRange = range !== null && range !== undefined
          && typeof range.start === 'number' && typeof range.end === 'number'
          && range.start >= 0 && range.end >= range.start && range.end <= baseDraft.length
        var prefix = hasRange ? baseDraft.slice(0, range.start) : ''
        var suffix = hasRange ? baseDraft.slice(range.end) : ''
        var startLen = prefix.length
        ...
        typewriterRef.current = window.setInterval(function () {
          ...
          try {
            actions.setDraft(prefix + fullText.slice(0, end) + suffix)
          } catch (_err) { /* ignore */ }
          if (textarea !== null) {
          ...
          if (ratio >= 1) {
            ...
            setTyping(false)
            setFormatBusy(false)
            focusComposer(startLen + fullText.length)
          }
        }, 20)
      }

      function runOptimizeMode(mode, draftOverride, ignorePhase, replaceRange, sourceText) {
        if (formatBusy) return
        if (ignorePhase !== true && phaseRef.current !== 'idle') return
        var actions = actionsRef.current
        if (actions === null || typeof actions.setDraft !== 'function') {
          setError(t('errNoInput'))
          return
        }
        var current = typeof draftOverride === 'string'
          ? draftOverride
          : (liveInputRef.current && typeof liveInputRef.current.draft === 'string'
            ? liveInputRef.current.draft
            : '')
        if (current.trim() === '') {
        ...
        // 自动优化只把“刚转写的那段”交给模型；手动优化处理整个输入框
        var text = typeof sourceText === 'string' && sourceText.trim() !== '' ? sourceText : current
        var caret = getComposerCaret(current.length)
        // 记录快照，供回退按钮恢复
        rollbackRef.current = { draft: current, caret: caret }
        setFormatBusy(true)
        setError(null)
        setInterim('')
        var contextText = conversationContext(sessionRef.current)
        var fixCfg = Object.assign({}, cfg, {
          formatterTemplate: cfg.formatterFixTemplate,
        })
        var finalCfg = Object.assign({}, cfg, {
          formatterWithContext: false,
          formatterTemplate: cfg.formatterFinalTemplate,
        })

        var request
        if (mode === 'round1') {
          request = formatDraftText(text, contextText, fixCfg, t)
            .then(function (result) { typewriterDraft(result, current, caret, replaceRange) })
        } else if (mode === 'round2') {
          request = formatDraftText(text, '', finalCfg, t)
            .then(function (result) { typewriterDraft(result, current, caret, replaceRange) })
        } else {
          request = formatDraftText(text, contextText, fixCfg, t).then(function (corrected) {
            if (!aliveRef.current) return null
            // 第二轮在原位置替换第一轮结果；基础草稿要先换成“第一轮已替换完成”的文本，
            // 否则纠错后文本变短/变长时会把原转写残留重新带出来
            var hasRange = replaceRange !== null && replaceRange !== undefined
              && typeof replaceRange.start === 'number' && typeof replaceRange.end === 'number'
            var correctedBase = hasRange
              ? current.slice(0, replaceRange.start) + corrected + current.slice(replaceRange.end)
              : corrected
            var nextRange = hasRange
              ? { start: replaceRange.start, end: replaceRange.start + corrected.length }
              : null
            return formatDraftText(corrected, '', finalCfg, t).then(function (formatted) {
              typewriterDraft(formatted, correctedBase, caret, nextRange)
            })
          })
        }
      ...
      function formatDraft() {
        runOptimizeMode(cfg.clickLeftMode || 'round1')
      }

      function twoPassFormatDraft() {
        runOptimizeMode(cfg.clickRightMode || 'round1+round2')
      }

      function rollbackDraft() {
        var snapshot = rollbackRef.current
        if (snapshot === null) return
        var actions = actionsRef.current
        if (actions === null || typeof actions.setDraft !== 'function') return
        actions.setDraft(snapshot.draft)
        rollbackRef.current = null
        focusComposer(snapshot.caret)
      }
      ...
      var hasText = composerDraft.trim() !== ''
      // 回退按钮与 AI 优化按钮绑定：文本发送/清空后一起隐藏；
      // 优化完成后（文本仍在、formatBusy=false）显示
      var canRollback = hasText && rollbackRef.current !== null

      return h('div', { className: 'vi_micAnchor', 'data-dsh-voice-input': '' },
        canRollback && !formatBusy ? h('button', {
          type: 'button',
          className: 'vi_micButton vi_rollbackButton',
          'aria-label': t('rollbackAria'),
    ...
    function VoiceSettingsPanel(props) {
      ...
      function checkField(key, label, hint, checked, onChange) {
        return h('div', { key: key, className: 'vi_field vi_fieldWide' },
          h('label', { className: 'vi_checkRow' },
            h('input', { type: 'checkbox', checked: checked === true, onChange: onChange }),
            h('span', null, label),
          ),
          h('span', { className: 'vi_hint' }, hint),
        )
      }

      function section(key, title, summary, open) {
        var children = Array.prototype.slice.call(arguments, 4)
        return h('details', { key: key, className: 'vi_section', open: open === true },
          h('summary', { className: 'vi_sectionSummary' },
            h('span', { className: 'vi_sectionSummaryHead' },
              h('span', { className: 'vi_sectionSummaryTitle' }, title),
              h('span', { className: 'vi_sectionSummaryDesc' }, summary),
            ),
          ),
          h.apply(null, ['div', { className: 'vi_sectionBody' }].concat(children)),
        )
      }

      function roundSelect(value, onChange) {
        return h('select', { className: 'vi_select', value: value, onChange: onChange },
          h('option', { value: 'round1' }, t('modeRound1')),
          h('option', { value: 'round2' }, t('modeRound2')),
          h('option', { value: 'round1+round2' }, t('modeRound1Plus2')),
        )
      }
      ...
      return h('div', { className: 'vi_root', 'data-dsh-voice-input-settings': '' },
        ...
        section('basics', t('basicsGroup'), t('basicsSummary'), true,
          h('div', { className: 'vi_grid' },
            ...
            checkField('shortcutEnabled', t('shortcutEnabled'), t('shortcutHint'),
              cfg.shortcutEnabled === true,
              function (event) { update({ shortcutEnabled: event.currentTarget.checked }) }),
          ),
        ),

        section('provider', t('providerGroup'), t('providerSummary'), true,
          h('h4', { className: 'vi_subTitle' }, t('providerFieldsSub')),
          h.apply(null, ['div', { className: 'vi_grid' }].concat(providerFields)),
          advancedFields.length > 0 ? h('details', { className: 'vi_field' },
            h('summary', { className: 'vi_label', style: { cursor: 'pointer' } }, t('advanced')),
            h.apply(null, ['div', { className: 'vi_grid', style: { marginTop: 8 } }].concat(advancedFields)),
          ) : null,
        ),

        section('formatter', t('formatterGroup'), t('formatterSummary'), false,
          h('p', { className: 'vi_hint' }, t('formatterIntro')),
          h('h4', { className: 'vi_subTitle' }, t('formatterTriggerSub')),
          h('div', { className: 'vi_grid' },
            checkField('autoOptimizeEnabled', t('autoOptimizeEnabled'), t('autoOptimizeEnabledHint'),
              cfg.autoOptimizeEnabled === true,
              function (event) { update({ autoOptimizeEnabled: event.currentTarget.checked }) }),
            field({
              label: t('autoMode'),
              hint: t('autoModeHint'),
              control: roundSelect(cfg.autoMode, function (event) { update({ autoMode: event.currentTarget.value }) }),
            }),
            field({
              label: t('clickLeftMode'),
              hint: t('clickLeftModeHint'),
              control: roundSelect(cfg.clickLeftMode, function (event) { update({ clickLeftMode: event.currentTarget.value }) }),
            }),
            field({
              label: t('clickRightMode'),
              hint: t('clickRightModeHint'),
              control: roundSelect(cfg.clickRightMode, function (event) { update({ clickRightMode: event.currentTarget.value }) }),
            }),
          ),
          h('h4', { className: 'vi_subTitle' }, t('formatterModelSub')),
          h('div', { className: 'vi_grid' },
            field({
            ...
            field({
              key: 'formatterThinking',
              label: t('formatterThinking'),
              hint: t('formatterThinkingHint'),
              control: h('select', {
                className: 'vi_select',
                value: cfg.formatterThinking,
                onChange: function (event) {
                  var value = event.currentTarget.value
                  // 同步旧版布尔字段，保证旧配置/外部工具读取时一致
                  update({ formatterThinking: value, formatterNoThinking: value === 'off' })
                },
              },
                h('option', { value: 'off' }, t('thinkingOff')),
                h('option', { value: 'low' }, t('thinkingLow')),
                h('option', { value: 'medium' }, t('thinkingMedium')),
                h('option', { value: 'high' }, t('thinkingHigh')),
              ),
            }),
            checkField('formatterWithContext', t('formatterWithContext'), t('formatterWithContextHint'),
              cfg.formatterWithContext === true,
              function (event) { update({ formatterWithContext: event.currentTarget.checked }) }),
          ),
          h('h4', { className: 'vi_subTitle' }, t('formatterPromptsSub')),
          field({
        ...
        section('import', t('importGroup'), t('importSummary'), false,
