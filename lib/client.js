window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      autoOptimizeEnabledHint: '开启后，语音识别完成即自动执行「自动优化模式」，不需要任何快捷键。',
      clickLeftMode: '左键模式',
    ...
    var en = {
      ...
      autoOptimizeEnabledHint: 'When enabled, each completed transcription is optimized automatically with the auto mode; no hotkeys are needed.',
    ...
    function VoiceInputButton(props) {
      ...
      function insertCleanText(clean) {
        ...
        focusComposer(prefix.length + sepBefore.length + clean.length)
        if (typeof document !== 'undefined') {
          var textarea = document.querySelector('textarea[data-phase]')
          if (textarea !== null) {
      ...
      function insertTranscript(text) {
        ...
        // 自动优化开关：转写完成即按「自动优化模式」直接优化，不再等用户点按钮
        if (cfg.autoOptimizeEnabled === true) {
          runOptimizeMode(cfg.autoMode || 'round1+round2', inserted, true)
        }
      }

      function typewriterDraft(fullText, baseDraft, caret) {
        var actions = actionsRef.current
        if (actions === null || typeof actions.setDraft !== 'function') {
          setFormatBusy(false)
          setTyping(false)
        ...
        var prefix = baseDraft.slice(0, caret)
        var suffix = baseDraft.slice(caret)
        var sepBefore = prefix !== '' && !/[\s\n\u3000]$/.test(prefix) ? ' ' : ''
        var sepAfter = suffix !== '' && !/^[\s\n\u3000]/.test(suffix) ? ' ' : ''
        var startLen = prefix.length + sepBefore.length
        ...
        typewriterRef.current = window.setInterval(function () {
          ...
          try {
            actions.setDraft(prefix + sepBefore + fullText.slice(0, end) + sepAfter + suffix)
          } catch (_err) { /* ignore */ }
          if (textarea !== null) {
      ...
      function runOptimizeMode(mode, draftOverride, ignorePhase) {
        ...
        var request
        if (mode === 'round1') {
          request = formatDraftText(current, contextText, fixCfg, t)
            .then(function (result) { typewriterDraft(result, current, caret) })
        } else if (mode === 'round2') {
          request = formatDraftText(current, '', finalCfg, t)
            .then(function (result) { typewriterDraft(result, current, caret) })
        } else {
          request = formatDraftText(current, contextText, fixCfg, t).then(function (corrected) {
            if (!aliveRef.current) return null
            if (cfg.formatterTwoPassEnabled === false) {
              typewriterDraft(corrected, current, caret)
              return null
            }
            return formatDraftText(corrected, '', finalCfg, t).then(function (formatted) {
              typewriterDraft(formatted, current, caret)
            })
          })
        }
