window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      formatterThinkingHint: '低档思考可提高输出准确性；关闭则最快。兼容 DeepSeek thinking 与 DashScope enable_thinking。',
      thinkingOff: '关闭思考',
      thinkingLow: '低',
      thinkingMedium: '中',
      thinkingHigh: '高',
      autoOptimizeEnabled: '自动优化开关',
      autoOptimizeEnabledHint: '开启后，语音转写完成会自动按「自动优化模式」优化文本，无需再点按钮。',
      clickLeftMode: '左键模式',
      clickRightMode: '右键模式',
      autoMode: '自动优化模式',
      modeRound1: '一轮（纠错）',
      ...
      hotkeyPass2: '二轮优化快捷键',
      hotkeyAuto: '自动优化快捷键',
      hotkeyHint: '点击输入框后按下组合键录制；例如 Alt+1、Ctrl+Alt+2。',
    ...
    var en = {
      ...
      formatterThinkingHint: 'Low thinking improves accuracy; off is fastest. Works with DeepSeek thinking and DashScope enable_thinking.',
      thinkingOff: 'Off',
      thinkingLow: 'Low',
      thinkingMedium: 'Medium',
      thinkingHigh: 'High',
      autoOptimizeEnabled: 'Auto optimize',
      autoOptimizeEnabledHint: 'When enabled, each completed transcription is optimized automatically using the auto mode.',
      ...
      hotkeyPass1: 'Round-1 hotkey',
      hotkeyPass2: 'Round-2 hotkey',
      hotkeyAuto: 'Auto optimize hotkey',
      hotkeyHint: 'Click the field and press a combo to record, e.g. Alt+1 or Ctrl+Alt+2.',
    ...
    function normalizeConfig(raw) {
      ...
      base.hotkeyPass1 = asString(src.hotkeyPass1, base.hotkeyPass1)
      base.hotkeyPass2 = asString(src.hotkeyPass2, base.hotkeyPass2)
      base.hotkeyAuto = asString(src.hotkeyAuto, base.hotkeyAuto)
    ...
    function VoiceInputButton(props) {
      ...
      react.useEffect(function () {
        ...
        var handle = {
          ...
          pass1: function () {
            var controller = controllerRef.current
            if (controller !== null) controller.optimize('round1')
          },
          pass2: function () {
            var controller = controllerRef.current
            if (controller !== null) controller.optimize('round1+round2')
          },
          auto: function () {
            var controller = controllerRef.current
            if (controller !== null) controller.optimize(getConfig().autoMode || 'round1+round2')
          },
        }
    ...
    function VoiceSettingsPanel(props) {
      ...
      function captureHotkey(event) {
        ...
        var key = null
        if (event.code && event.code.indexOf('Key') === 0) key = event.code.slice(3)
        else if (event.code && event.code.indexOf('Digit') === 0) key = event.code.slice(5)
      ...
      function hotkeyInput(value, fieldName) {
        return h('input', {
          className: 'vi_input', value: value, readOnly: true,
          placeholder: t('hotkeyHint'),
          onKeyDown: function (event) {
            var combo = captureHotkey(event)
            if (combo !== null) {
              var patch = {}
              patch[fieldName] = combo
              update(patch)
            }
          },
        })
      ...
      return h('div', { className: 'vi_root', 'data-dsh-voice-input-settings': '' },
        ...
        h('section', { className: 'vi_group' },
          ...
          h('div', { className: 'vi_grid' },
            ...
            h('div', { key: 'formatterTwoPassEnabled', className: 'vi_field vi_fieldWide' },
              h('label', { className: 'vi_checkRow' },
                h('input', {
                  type: 'checkbox',
                  checked: cfg.formatterTwoPassEnabled === true,
                  onChange: function (event) { update({ formatterTwoPassEnabled: event.currentTarget.checked }) },
                }),
                h('span', null, t('formatterTwoPassEnabled')),
              ),
              h('span', { className: 'vi_hint' }, t('formatterTwoPassEnabledHint')),
            ),
          ),
          h('div', { className: 'vi_grid' },
            field({
              ...
              control: h('select', {
                className: 'vi_select', value: cfg.clickLeftMode,
                onChange: function (event) { update({ clickLeftMode: event.currentTarget.value }) },
              },
            ...
            field({
              ...
              control: h('select', {
                className: 'vi_select', value: cfg.clickRightMode,
                onChange: function (event) { update({ clickRightMode: event.currentTarget.value }) },
              },
                h('option', { value: 'round1' }, t('modeRound1')),
                h('option', { value: 'round2' }, t('modeRound2')),
                h('option', { value: 'round1+round2' }, t('modeRound1Plus2')),
            ...
            h('div', { key: 'autoOptimizeEnabled', className: 'vi_field vi_fieldWide' },
              h('label', { className: 'vi_checkRow' },
                h('input', {
                  type: 'checkbox',
                  checked: cfg.autoOptimizeEnabled === true,
                  onChange: function (event) { update({ autoOptimizeEnabled: event.currentTarget.checked }) },
                }),
                h('span', null, t('autoOptimizeEnabled')),
              ),
              h('span', { className: 'vi_hint' }, t('autoOptimizeEnabledHint')),
            ),
            field({
              label: t('autoMode'),
              control: h('select', {
                className: 'vi_select', value: cfg.autoMode,
                onChange: function (event) { update({ autoMode: event.currentTarget.value }) },
              },
                h('option', { value: 'round1' }, t('modeRound1')),
                h('option', { value: 'round2' }, t('modeRound2')),
                h('option', { value: 'round1+round2' }, t('modeRound1Plus2')),
            ...
            field({ label: t('hotkeyPass1'), hint: t('hotkeyHint'), control: hotkeyInput(cfg.hotkeyPass1, 'hotkeyPass1') }),
            field({ label: t('hotkeyPass2'), hint: t('hotkeyHint'), control: hotkeyInput(cfg.hotkeyPass2, 'hotkeyPass2') }),
            field({ label: t('hotkeyAuto'), hint: t('hotkeyHint'), control: hotkeyInput(cfg.hotkeyAuto, 'hotkeyAuto') }),
          ),
          h('div', { className: 'vi_grid' },
            field({
    ...
    function apply(ctx) {
      ...
      // 文本整理快捷键（可在设置中自定义）
      ctx.effect(function () {
        function comboMatches(event, spec) {
          ...
          if (key === '') return false
          if (event.ctrlKey !== mods.Ctrl || event.altKey !== mods.Alt || event.shiftKey !== mods.Shift || event.metaKey !== mods.Meta) return false
          var norm = ''
          if (event.code && event.code.indexOf('Key') === 0) norm = event.code.slice(3)
          else if (event.code && event.code.indexOf('Digit') === 0) norm = event.code.slice(5)
          else if (event.code === 'Space') norm = 'Space'
          else norm = event.key || event.code || ''
          return norm.toLowerCase() === key.toLowerCase()
        }
        function onHotkeyDown(event) {
          var currentConfig = getConfig()
          var controller = voiceController.current
          if (controller === null) return
          if (comboMatches(event, currentConfig.hotkeyPass1)) {
            event.preventDefault()
            controller.pass1()
          } else if (comboMatches(event, currentConfig.hotkeyPass2)) {
            event.preventDefault()
            controller.pass2()
          } else if (comboMatches(event, currentConfig.hotkeyAuto)) {
            event.preventDefault()
            controller.auto()
          }
        }
      ...
      }, 'dsh-voice-input: formatter hotkeys')
