window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      formatterIntro: '语音识别完成后可以自动优化，也可以点 ✨ / ⚡ 手动优化。左键默认一轮纠错；右键默认一轮纠错 + 二轮固定格式。结果自动写回输入框。',
      ...
      formatterTwoPassEnabled: '启用右键第二轮优化',
      formatterTwoPassEnabledHint: '关闭后，右键只执行第一轮纠错，不再调用第二轮生成固定格式；左键行为不变。',
      ...
      clickLeftMode: '左键模式',
      clickLeftModeHint: '左键 ✨ 执行什么：一轮只纠错；二轮只套固定格式；一轮+二轮先纠错再格式化。',
      clickRightMode: '右键模式',
      clickRightModeHint: '右键 ✨ 执行什么，默认先纠错再格式化。',
      autoMode: '自动优化模式',
      autoModeHint: '自动优化开关打开后，语音转写完成自动执行的流程。',
      modeRound1: '一轮（纠错）',
      modeRound2: '二轮（固定格式）',
      modeRound1Plus2: '一轮 + 二轮',
      autoOptimizeAria: '自动优化文本',
      autoOptimizeTitle: '按设置模式自动优化',
    ...
    var en = {
      ...
      formatterIntro: 'After speech recognition the text can be optimized automatically, or manually with ✨ / ⚡. Left-click: round-1 fix by default. Right-click: fix then fixed-format by default. The result is written back automatically.',
      ...
      formatterTwoPassEnabled: 'Enable right-click second round',
      formatterTwoPassEnabledHint: 'When off, right-click runs only the typo-fix round and skips the format round; left-click is unchanged.',
      ...
      clickLeftModeHint: 'What the left-click ✨ does: round 1 fixes only, round 2 applies the fixed format only, round 1+round 2 fixes then formats.',
      clickRightMode: 'Right-click mode',
      clickRightModeHint: 'What the right-click ✨ does; fix then format by default.',
      ...
      autoOptimizeAria: 'Auto optimize text',
      autoOptimizeTitle: 'Auto optimize using the selected mode',
    ...
    function normalizeConfig(raw) {
      ...
      base.formatterTwoPassEnabled = typeof src.formatterTwoPassEnabled === 'boolean' ? src.formatterTwoPassEnabled : base.formatterTwoPassEnabled
    ...
    function VoiceInputButton(props) {
      ...
      function runOptimizeMode(mode, draftOverride, ignorePhase, replaceRange, sourceText) {
        ...
        if (mode === 'round1') {
          ...
          request = formatDraftText(text, contextText, fixCfg, t).then(function (corrected) {
            ...
            if (cfg.formatterTwoPassEnabled === false) {
              typewriterDraft(corrected, current, caret, replaceRange)
              return null
            }
      ...
      function autoOptimizeDraft() {
        runOptimizeMode(cfg.autoMode || 'round1+round2')
      }
      ...
      var canRollback = rollbackRef.current !== null
      ...
      return h('div', { className: 'vi_micAnchor', 'data-dsh-voice-input': '' },
        ...
        hasText ? h('button', {
          type: 'button',
          className: 'vi_micButton vi_autoButton',
          'aria-label': t('autoOptimizeAria'),
          title: t('autoOptimizeTitle'),
          disabled: formatBusy || phase !== 'idle',
          onClick: autoOptimizeDraft,
        }, h('span', { 'aria-hidden': 'true' }, '⚡')) : null,
        hasText ? h('button', {
          type: 'button',
      ...
      return h('div', { className: 'vi_root', 'data-dsh-voice-input-settings': '' },
        ...
        section('formatter', t('formatterGroup'), t('formatterSummary'), false,
          ...
          h('div', { className: 'vi_grid' },
            ...
            checkField('formatterTwoPassEnabled', t('formatterTwoPassEnabled'), t('formatterTwoPassEnabledHint'),
              cfg.formatterTwoPassEnabled === true,
              function (event) { update({ formatterTwoPassEnabled: event.currentTarget.checked }) }),
          ),
