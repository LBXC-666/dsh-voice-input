window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      /* 录音蓝色波纹 / 整理彩虹流光：沿边框旋转 + 上下起伏 + 音量震动 */
      '.vi_flow-host{--vi-wave-scale:1;--vi-wave-shake:0px;position:relative}',
      '.vi_flow-overlay{position:absolute;inset:-3px;z-index:30;pointer-events:none;border-radius:inherit;padding:3px;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;overflow:hidden;animation:vi_ripple_shake .55s ease-in-out infinite}',
      '@property --vi-angle{syntax:"<angle>";initial-value:0deg;inherits:false}',
      '.vi_flow-wave{position:absolute;inset:0}',
      '.vi_flow-wave-a{background:repeating-conic-gradient(from var(--vi-angle,0deg),rgba(10,132,255,.1) 0deg 14deg,rgba(10,132,255,.9) 14deg 20deg,rgba(90,209,255,.85) 20deg 24deg,rgba(10,132,255,.1) 24deg 42deg);animation:vi_flow_spin 1.8s linear infinite}',
      '.vi_flow-wave-b{background:repeating-linear-gradient(115deg,transparent 0 14px,rgba(0,176,255,.55) 14px 17px,transparent 17px 36px);background-size:260% 100%;animation:vi_flow_b 2.2s linear infinite}',
      '.vi_flow-overlay[data-flow=rainbow] .vi_flow-wave-a{background:conic-gradient(from var(--vi-angle,0deg),rgba(255,0,76,.32),rgba(255,138,0,.36),rgba(255,230,0,.32),rgba(0,230,118,.32),rgba(0,176,255,.36),rgba(124,77,255,.32),rgba(255,0,76,.32));animation:vi_flow_spin 2.5s linear infinite}',
      '.vi_flow-overlay[data-flow=rainbow] .vi_flow-wave-b{background:linear-gradient(90deg,rgba(255,0,76,.18),rgba(0,176,255,.2),rgba(0,230,118,.18),rgba(255,0,76,.18));background-size:260% 100%;animation:vi_flow_b 3.6s linear infinite reverse}',
      '@keyframes vi_flow_spin{to{--vi-angle:360deg}}',
      '@keyframes vi_flow_b{to{background-position:260% 0}}',
      '@keyframes vi_ripple_shake{0%,100%{transform:translateX(calc(var(--vi-wave-shake,0px)*-1)) translateY(-2px) scale(var(--vi-wave-scale,1))}25%{transform:translateX(var(--vi-wave-shake,0px)) translateY(0) scale(var(--vi-wave-scale,1))}50%{transform:translateX(calc(var(--vi-wave-shake,0px)*-1)) translateY(2px) scale(var(--vi-wave-scale,1))}75%{transform:translateX(var(--vi-wave-shake,0px)) translateY(0) scale(var(--vi-wave-scale,1))}}',
      /* 打字机：彩虹一行一行从上往下扫过，旧字保持原色 */
      '.vi_typing-host{position:relative}',
      '.vi_typing-host::after{content:"";position:absolute;left:0;right:0;top:0;height:26px;border-radius:999px;pointer-events:none;background:linear-gradient(180deg,transparent,rgba(255,0,76,.08),rgba(255,138,0,.1),rgba(255,230,0,.1),rgba(0,230,118,.08),rgba(0,176,255,.1),rgba(124,77,255,.08),transparent);filter:blur(8px);opacity:.32;animation:vi_typing_sweep 1.1s ease-in-out}',
      '@keyframes vi_typing_sweep{0%{top:-14px;opacity:.05;transform:scaleY(.8)}25%{opacity:.35;transform:scaleY(1)}100%{top:calc(100% - 26px);opacity:0;transform:scaleY(1.2)}}',
    ...
    function VoiceInputButton(props) {
      ...
      react.useEffect(function () {
        ...
        return function () {
          ...
          host.style.setProperty('--vi-wave-scale', '1')
          host.style.setProperty('--vi-wave-shake', '0px')
        }
      }, [flowKind])
            ...
            node.port.onmessage = function (event) {
              ...
              if (event.data !== null && typeof event.data === 'object' && event.data.type === 'level') {
                ...
                if (host !== null) {
                  ...
                  host.style.setProperty('--vi-wave-scale', (1 + normalized * 0.06).toFixed(4))
                  host.style.setProperty('--vi-wave-shake', (normalized * 6).toFixed(2) + 'px')
                }
                return
              }
