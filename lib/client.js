window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      /* AI 整理期间的彩虹流光边缘 */
      '.vi_rainbow-host{position:relative}',
      '.vi_rainbow-overlay{position:absolute;inset:-2px;z-index:30;pointer-events:none;border-radius:14px;padding:2px;background:conic-gradient(from var(--vi-angle,0deg),#ff004c,#ff8a00,#ffe600,#00e676,#00b0ff,#7c4dff,#ff004c);-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;animation:vi_rainbow_spin 2.5s linear infinite}',
      '@property --vi-angle{syntax:"<angle>";initial-value:0deg;inherits:false}',
      '@keyframes vi_rainbow_spin{to{--vi-angle:360deg}}',
    ...
    function formatDraftText(text, contextText, cfg, t) {
      ...
      var context = cfg.formatterWithContext !== false ? (contextText || '') : ''
      ...
      if (cfg.formatterNoThinking !== false) {
    ...
    function VoiceInputButton(props) {
      ...
      // AI 整理期间：给输入框卡片加彩虹流光边缘
      react.useEffect(function () {
        if (!formatBusy) return undefined
        var host = composerHostElement()
        if (host === null) return undefined
        host.classList.add('vi_rainbow-host')
        var overlay = document.createElement('div')
        overlay.className = 'vi_rainbow-overlay'
        overlay.setAttribute('aria-hidden', 'true')
        host.appendChild(overlay)
        return function () {
          if (overlay.parentNode === host) host.removeChild(overlay)
          host.classList.remove('vi_rainbow-host')
        }
      }, [formatBusy])
