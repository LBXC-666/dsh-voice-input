window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      formatterGroup: '文本整理（双击右 Alt 录音后整理）',
      formatterEnabled: '启用文本整理',
      formatterEnabledHint: '关闭后，双击右 Alt 与普通录音完全相同：识别结果直接插入，不调用整理模型。',
      formatterIntro: '快速双击右 Alt 开始录音（第二次按住说话），松手后先语音识别，再把识别文本发送给模型清洗并格式化为「目标 / 关键要求 / 补充信息」，结果自动写回输入框。',
    ...
    var en = {
      ...
      formatterGroup: 'Text formatting (double-tap Right Alt dictation)',
      formatterEnabled: 'Enable text formatting',
      formatterEnabledHint: 'When off, double-tapping Right Alt behaves exactly like normal recording: the transcript is inserted directly without calling the formatting model.',
      formatterIntro: 'Quickly double-tap Right Alt and hold the second press while talking; on release the transcript is sent to the model, cleaned and formatted as goal / key requirements / extra info, then written back into the composer.',
    ...
    function normalizeConfig(raw) {
      ...
      base.formatterEnabled = typeof src.formatterEnabled === 'boolean' ? src.formatterEnabled : base.formatterEnabled
    ...
    function VoiceInputButton(props) {
      ...
      var formatAfterRef = react.useRef(false)
      var lastAltTapRef = react.useRef(0)
      ...
      function runFormatThenInsert(clean) {
        setFormatBusy(true)
        setError(null)
        setInterim('')
        formatDraftText(clean, cfg, t).then(function (formatted) {
          if (!aliveRef.current) return
          setFormatBusy(false)
          insertCleanText(formatted)
        }).catch(function (error) {
          if (!aliveRef.current) return
          setFormatBusy(false)
      ...
      function insertTranscript(text) {
        ...
        if (formatAfterRef.current) {
          // Alt+Space 录音：识别完成后先整理再写入
          formatAfterRef.current = false
          runFormatThenInsert(clean)
          return
        }
      ...
      // 右 Alt：按住说话；快速双击右 Alt：识别完成后自动整理文本
      function startFromShortcut(opts) {
        if (phaseRef.current !== 'idle') return
        var now = Date.now()
        var doubleTap = now - lastAltTapRef.current <= 450
        lastAltTapRef.current = now
        shortcutActiveRef.current = true
        formatAfterRef.current = doubleTap && cfg.formatterEnabled === true
        startRecording()
      }
    ...
    function VoiceSettingsPanel(props) {
      ...
      return h('div', { className: 'vi_root', 'data-dsh-voice-input-settings': '' },
        ...
        h('section', { className: 'vi_group' },
          ...
          h('div', { className: 'vi_grid' },
            h('div', { key: 'formatterEnabled', className: 'vi_field vi_fieldWide' },
              h('label', { className: 'vi_checkRow' },
                h('input', {
                  type: 'checkbox',
                  checked: cfg.formatterEnabled === true,
                  onChange: function (event) { update({ formatterEnabled: event.currentTarget.checked }) },
                }),
                h('span', null, t('formatterEnabled')),
              ),
              h('span', { className: 'vi_hint' }, t('formatterEnabledHint')),
            ),
          ),
          h('div', { className: 'vi_grid' },
            field({
