window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      '.vi_pop.vi_live{min-width:0}',
      '.vi_live{max-width:320px;max-height:240px;overflow-y:auto;overscroll-behavior:contain;color:var(--dsw-alias-label-primary);white-space:pre-wrap;overflow-wrap:anywhere}',
      ...
      /* 录音蓝色波纹 / 整理彩虹流光：双层交织 + 音量震动 */
      '.vi_flow-host{--vi-wave-scale:1;--vi-wave-shake:0px;position:relative}',
      '.vi_flow-overlay{position:absolute;inset:-2px;z-index:30;pointer-events:none;border-radius:14px;padding:2px;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;overflow:hidden;animation:vi_ripple_shake .18s linear infinite}',
      '.vi_flow-wave{position:absolute;inset:0}',
      '.vi_flow-wave-a{background:repeating-linear-gradient(90deg,rgba(10,132,255,.1) 0 10px,#0a84ff 10px 13px,#5ad1ff 13px 16px,rgba(10,132,255,.1) 16px 28px);background-size:220% 100%;animation:vi_flow_a 1.1s linear infinite}',
      '.vi_flow-wave-b{background:repeating-linear-gradient(115deg,transparent 0 16px,rgba(0,176,255,.55) 16px 19px,transparent 19px 38px);background-size:260% 100%;animation:vi_flow_b 2.2s linear infinite}',
      '.vi_flow-overlay[data-flow=rainbow] .vi_flow-wave-a{background:linear-gradient(90deg,#ff004c,#ff8a00,#ffe600,#00e676,#00b0ff,#7c4dff,#ff004c);background-size:220% 100%;animation:vi_flow_a 2.5s linear infinite}',
      '.vi_flow-overlay[data-flow=rainbow] .vi_flow-wave-b{background:linear-gradient(90deg,rgba(255,0,76,.5),rgba(0,176,255,.5),rgba(0,230,118,.5),rgba(255,0,76,.5));background-size:260% 100%;animation:vi_flow_b 3.6s linear infinite reverse}',
      '@keyframes vi_flow_a{to{background-position:-220% 0}}',
      '@keyframes vi_flow_b{to{background-position:260% 0}}',
      '@keyframes vi_ripple_shake{0%,100%{transform:translateX(calc(var(--vi-wave-shake,0px)*-1)) scale(var(--vi-wave-scale,1))}50%{transform:translateX(var(--vi-wave-shake,0px)) scale(var(--vi-wave-scale,1))}}',
      /* 打字机：彩虹光带扫过推动新字，旧字保持原色 */
      '.vi_typing-host{position:relative}',
      '.vi_typing-host::after{content:"";position:absolute;top:4px;bottom:4px;left:-10%;width:72px;border-radius:999px;pointer-events:none;background:linear-gradient(90deg,transparent,#ff004c,#ff8a00,#ffe600,#00e676,#00b0ff,#7c4dff,transparent);filter:blur(5px);opacity:.55;animation:vi_typing_sweep .95s ease-in-out}',
      '@keyframes vi_typing_sweep{0%{left:-10%;opacity:.1}25%{opacity:.7}100%{left:calc(100% - 72px);opacity:0}}',
            ...
            node.port.onmessage = function (event) {
              ...
              if (event.data !== null && typeof event.data === 'object' && event.data.type === 'level') {
                ...
                if (host !== null) {
                  ...
                  host.style.setProperty('--vi-wave-scale', (1 + normalized * 0.03).toFixed(4))
                  host.style.setProperty('--vi-wave-shake', (normalized * 3.5).toFixed(2) + 'px')
                }
                return
              }
