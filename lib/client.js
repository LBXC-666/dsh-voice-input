window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      formatterGroup: '文本整理（Alt+Space）',
      formatterIntro: '选中对话输入框后按 Alt+Space：把输入框里的原始文本发送给模型，清洗并格式化为“目标 / 关键要求 / 补充信息”，结果自动写回输入框。',
    ...
    var en = {
      ...
      formatterGroup: 'Text formatting (Alt+Space)',
      formatterIntro: 'Press Alt+Space while the composer has text: the draft is sent to the model, cleaned and formatted as goal / key requirements / extra info, then written back into the composer.',
    ...
    function apply(ctx) {
      ...
      // Alt+Space：把输入框原文交给模型清洗并格式化，结果写回输入框
      ctx.effect(function () {
      ...
      }, 'dsh-voice-input: alt+space format shortcut')
