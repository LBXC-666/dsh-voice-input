window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      formatterGroup: '文本整理（Alt+Space 录音后整理）',
      formatterIntro: '按住 Alt+Space 说话，松手后先语音识别，再把识别文本发送给模型清洗并格式化为「目标 / 关键要求 / 补充信息」，结果自动写回输入框。',
    ...
    var en = {
      ...
      formatterGroup: 'Text formatting (Alt+Space dictation then format)',
      formatterIntro: 'Hold Alt+Space and talk; on release the transcript is sent to the model, cleaned and formatted as goal / key requirements / extra info, then written back into the composer.',
    ...
    function VoiceInputButton(props) {
      ...
      // 右 Alt / Alt+Space 快捷键：keydown 开始，keyup 结束
      function startFromShortcut(opts) {
        if (phaseRef.current !== 'idle') return
        shortcutActiveRef.current = true
        formatAfterRef.current = !!(opts && opts.formatAfter)
        startRecording()
      }
    ...
    function apply(ctx) {
      ...
      // Alt+Space：与右 Alt 一样按住录音，松手后识别完成会自动整理文本
      ctx.effect(function () {
        function onAltSpace(event) {
          ...
          event.preventDefault()
          var controller = voiceController.current
          if (controller !== null) controller.start({ formatAfter: true })
        }
        function onAltSpaceKeyUp(event) {
          if (event.code !== 'Space') return
          var controller = voiceController.current
          if (controller !== null) controller.stop()
        }
        window.addEventListener('keydown', onAltSpace, true)
        window.addEventListener('keyup', onAltSpaceKeyUp, true)
        return function () {
          window.removeEventListener('keydown', onAltSpace, true)
          window.removeEventListener('keyup', onAltSpaceKeyUp, true)
        }
      }, 'dsh-voice-input: alt+space record-and-format shortcut')
