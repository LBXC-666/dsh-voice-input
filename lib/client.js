window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      '.vi_micButton{width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center;border-radius:8px;border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer;padding:0;flex:none}',
      '.vi_micButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      ...
      '.vi_micButton[data-phase=recording]{color:var(--dsw-alias-state-error-primary);border-color:color-mix(in srgb, var(--dsw-alias-state-error-primary) 60%, transparent);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 10%, transparent)}',
      '.vi_micButton[data-phase=recording]:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 16%, transparent);color:var(--dsw-alias-state-error-primary)}',
      '.vi_pop{position:absolute;bottom:calc(100% + 8px);right:0;z-index:60;min-width:max-content;max-width:260px;display:flex;align-items:center;gap:7px;background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l1);border-radius:10px;box-shadow:var(--dsw-shadow-lv2);padding:7px 10px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-primary)}',
      ...
      '.vi_spin{width:13px;height:13px;border:2px solid color-mix(in srgb, var(--dsw-alias-label-tertiary) 40%, transparent);border-top-color:var(--dsw-alias-label-primary);border-radius:999px;flex:none;animation:vi_spin .8s linear infinite}',
    ...
    var zh = {
      ...
      shortcutHint: '在页面任意位置按右 Alt 开始录音，再按一次停止并转写。AltGr 键盘布局、组合键与按住重复触发会被自动忽略。',
      ...
      errConfig: '请先在 设置 → 插件 → 语音输入 中配置接口参数。',
    ...
    var en = {
      ...
      shortcutHint: 'Press Right Alt anywhere in the page to start recording, and press it again to stop and transcribe. AltGr layouts, key combos and key-repeat are ignored automatically.',
      ...
      errConfig: 'Configure the endpoint in Settings → Plugins → Voice input first.',
    ...
    var DEFAULT_CONFIG = {
      mode: 'openai', // openai | custom-json | browser
      preset: 'openai',
    ...
    function normalizeConfig(raw) {
      ...
      var modes = ['openai', 'custom-json', 'browser']
    ...
    function StopIcon(props) {
      return react.createElement('svg', Object.assign({}, props, {
        ...
        react.createElement('rect', { x: 3.5, y: 3.5, width: 9, height: 9, rx: 2, fill: 'currentColor' }),
      )
    }
    ...
    function VoiceInputButton(props) {
      ...
      controllerRef.current = { toggle: onButtonClick }
      ...
      if (phase === 'recording') {
        popover = h('div', { className: 'vi_pop', role: 'status' },
          h('span', { className: 'vi_recDot', 'aria-hidden': 'true' }),
          h('span', null, t('recording') + ' ' + formatSeconds(elapsed)),
        )
      } else if (phase === 'processing') {
        popover = h('div', { className: 'vi_pop', role: 'status' },
          h('span', { className: 'vi_spin', 'aria-hidden': 'true' }),
          h('span', null, t('processing')),
        )
      } else if (error !== null) {
      ...
      return h('div', { className: 'vi_micAnchor', 'data-dsh-voice-input': '' },
        ...
        }, phase === 'recording' ? h(StopIcon, {}) : phase === 'processing' ? h('span', { className: 'vi_spin' }) : h(MicIcon, {})),
        popover,
      )
    }
    ...
    function apply(ctx) {
      ...
      // 全局快捷键：右 Alt 开始/停止语音输入（自动忽略 AltGr、组合键与长按重复）
      ctx.effect(function () {
        function onRightAltKeyDown(event) {
          var isRightAlt = event.code === 'AltRight' || (event.key === 'Alt' && event.location === 2)
      ...
      }, 'dsh-voice-input: right-alt shortcut')
