window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      '.vi_live{max-width:320px;max-height:240px;overflow-y:auto;overscroll-behavior:contain;color:var(--dsw-alias-label-primary);white-space:pre-wrap}',
      ...
      /* AI 整理期间的彩虹流光边缘 / 录音期间的蓝色波纹边缘 */
      '.vi_flow-host{position:relative}',
      ...
      '.vi_flow-overlay[data-flow=blue]{background:repeating-linear-gradient(90deg,rgba(10,132,255,.12) 0 8px,#0a84ff 8px 12px,#5ad1ff 12px 16px,rgba(10,132,255,.12) 16px 24px);background-size:200% 100%;animation:vi_flow_blue 1.4s linear infinite}',
      '.vi_flow-overlay[data-flow=rainbow]{background:linear-gradient(90deg,#ff004c,#ff8a00,#ffe600,#00e676,#00b0ff,#7c4dff,#ff004c);background-size:200% 100%;animation:vi_flow_rainbow 2.5s linear infinite}',
      '@keyframes vi_flow_blue{to{background-position:-200% 0}}',
      '@keyframes vi_flow_rainbow{to{background-position:-200% 0}}',
      /* 打字机输出期间的文字效果：内部彩虹纹理划过 + 运动模糊光晕 */
      '.vi_typing-text{background-image:linear-gradient(90deg,#ff004c,#ff8a00,#ffe600,#00e676,#00b0ff,#7c4dff,#ff004c);background-size:300% 100%;-webkit-background-clip:text;background-clip:text;color:transparent;-webkit-text-fill-color:transparent;caret-color:#00b0ff;animation:vi_text_rainbow 2.2s linear infinite,vi_text_blur .35s ease-in-out infinite alternate}',
      '@keyframes vi_text_rainbow{to{background-position:300% 0}}',
      '@keyframes vi_text_blur{from{text-shadow:-3px 0 6px rgba(0,176,255,.35),3px 0 6px rgba(255,0,76,.35)}to{text-shadow:3px 0 6px rgba(0,230,118,.35),-3px 0 6px rgba(124,77,255,.35)}}',
    ...
    function VoiceInputButton(props) {
      ...
      // 打字机输出期间：给输入框文字加彩虹纹理 + 运动模糊光晕
      react.useEffect(function () {
        if (!typing) return undefined
        var textarea = document.querySelector('textarea[data-phase]')
        if (textarea === null) return undefined
        textarea.classList.add('vi_typing-text')
        return function () {
          textarea.classList.remove('vi_typing-text')
        }
      }, [typing])
