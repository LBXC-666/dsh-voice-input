window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      formatterWithContextHint: '把最近几条对话的文本（用户/助手消息，各截断 300 字）附加到整理请求里，帮助理解指代与联想；不发送附件。',
    ...
    var en = {
      ...
      formatterWithContextHint: 'Appends the latest user/assistant text messages (each truncated to 300 chars) to the formatting request to help resolve pronouns and references; attachments are not sent.',
    ...
    function conversationContext(snapshot) {
      ...
      var lines = []
      var count = 0
      for (var i = snapshot.nodes.length - 1; i >= 0 && count < 8; i--) {
