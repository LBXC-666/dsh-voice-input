...
- ✨ **文本整理（双击右 Alt）**：语音录入后自动交给 LLM 清洗并格式化为「目标 / 关键要求 / 补充信息」，提示词模板可自定义，结果自动写回输入框。
...
### 文本整理：双击右 Alt（录音后自动整理）

把“发给 agent 之前的语音输入”识别并格式化：

1. **快速双击右 Alt，第二次按住说话**；
2. 松手后先完成语音识别；
3. 再把识别文本发送给你配置的 OpenAI 兼容模型（`POST /chat/completions`）；
4. 模型按提示词模板输出整理后的文本，并**直接写回输入框**。
