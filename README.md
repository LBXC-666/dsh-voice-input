...
1. 语音识别完成后，🎤 左侧弹出 **✨ AI 按钮**；
2. **左键单击 ✨**：单轮整理（模板 + 可选上下文）→ 写回输入框；
3. **右键单击 ✨**：两轮处理——第一轮带上下文纠错，第二轮不带上下文套固定格式 → 写回输入框；
4. 结果自动**替换输入框内容**，整理完成后按钮消失；
5. 手动打字 / 点击发送 / 开始新一轮录音时，按钮自动收回。
...
| 启用右键第二轮优化 | 默认开启；关闭后右键只做第一轮纠错，不调用第二轮 |
